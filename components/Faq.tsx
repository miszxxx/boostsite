      ...
      <div className="middle flex items-center flex-col relative">
        <RevealAnimation screenReveal>
          <div className="inline-flex items-center gap-2 rounded-full border border-[#f4adfb]/20 bg-gradient-to-r from-[#f4adfb]/[0.05] to-[#5b2873]/[0.05] px-6 py-3 backdrop-blur-sm">
            <MessageCircleQuestionIcon size={16} className="text-[#5b2873]" />
          ...
          <Accordion type="single" collapsible className="w-full space-y-4">
            {FaqData.map((faq, index) => (
              <RevealAnimation screenReveal delay={index * 0.2} key={faq.id}>
                <AccordionItem 
                  value={faq.id}
                  className="border border-[#f4adfb]/20 rounded-lg bg-gradient-to-r from-[#f4adfb]/[0.05] to-[#5b2873]/[0.05] backdrop-blur-sm transition-all duration-300 hover:border-[#f4adfb]/40"
                >
                  <AccordionTrigger 
                    className="group text-lg text-start text-white hover:bg-gradient-to-r hover:from-[#f4adfb] hover:via-[#5b2873] hover:to-[#f4adfb] hover:bg-clip-text hover:text-transparent transition-all py-4 px-6 font-medium"
                  >
                    {faq.question}
                  </AccordionTrigger>
